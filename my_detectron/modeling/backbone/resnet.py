...
from my_detectron.modeling.backbone.build import BACKBONE_REGISTRY
...
# from my_detectron.layers.blocks import CNNBlockBase
# from my_detectron.layers.wrappers import Conv2d
...
class ResNet(Backbone):
    ...
    def forward(self, x):
        """
        Args:
            x: Tenosr of shape (N, C, H, W). H, W must be a multiple of ``self.size_divisibility``

        Returns:
            dict[str->Tensor]: names and corresponding features
        """
        assert (
            x.dim() == 4
...
def build_resnet_backbone(cfg, input_shape):
    ...
    for idx, stage_idx in enumerate(range(2, 6)):
        ...
        first_stide = 1 if idx == 0 or (stage_idx == 5 and dilation == 2) else 2
        stage_kargs = {
            "num_blocks": num_blocks_per_stage[idx],
            "stride_per_block": [first_stide] + [1] * (num_blocks_per_stage[idx] - 1),
        ...
        if depth in [18, 34]:
            ...
            if deform_on_per_stage[idx]:
                stage_kargs["block_class"] = DeformBottleneckBlcok
