...
def permute_to_N_HWA_K(tensor, K: int):
    ...
    # first transpose to (N, Ai, K, H, W)
    tensor = tensor.view(N, -1, K, H, W)
    # permute to (N, H, W, Ai, K)
...
class DenseDetector(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):
        ...
        if self.training:
            ...
            for results_per_image, input_per_image, image_size in zip(
                results, batched_inputs, images.image_size
            ):
    ...
    def _decode_per_level_predictions(
        ...
        anchor_idxs, classes_idxes = topk_idxs.unbind(dim=1)
