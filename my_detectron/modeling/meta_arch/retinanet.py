import math
import logging
from detectron2.layers.wrappers import Conv2d, nonzero_tuple
...
from detectron2.structures import Boxes, ImageList, image_list, pairwise_iou, Instances
from detectron2.utils.events import get_event_storage
from detectron2.layers import cat, ShapeSpec, get_norm, batched_nms
from detectron2.modeling.postprocessing import detector_postprocess
from detectron2.config import configurable
from detectron2.modeling.backbone import Backbone
from torchvision.ops import boxes

from my_detectron.modeling.meta_arch.build import META_ARCH_REGISTRY
from my_detectron.modeling.backbone.build import build_backbone
from my_detectron.modeling.box_regression import (
    _dense_box_regression_loss,
    Box2BoxTransform,
)
from my_detectron.modeling.matcher import Matcher
from my_detectron.modeling.anchor_generator import build_anchor_generator
...
class RetinaNet(nn.Module):
    ...
    def label_anchors(self, anchors, gt_instances):
        ...
        for gt_per_image in gt_instances:
            ...
            matched_idx, anchor_labels = self.anchor_matcher(match_quality_matrix)
            del match_quality_matrix

            if len(gt_per_image) > 0:
                matched_gt_boxes_i = gt_per_image.gt_boxes.tensor[matched_idx]

                gt_labels_i = gt_per_image.gt_classes[matched_idx]
                gt_labels_i[anchor_labels == 0] = self.num_classes
                gt_labels_i[anchor_labels == -1] = -1
            else:
                matched_gt_boxes_i = torch.zeros_like(anchors.tensor)
                gt_labels_i = torch.zeros_like(matched_idx) + self.num_classes
    ...
    def inference_single_image(
        ...
        for box_cls_i, box_reg_i, anchors_i in zip(box_cls, box_delta, anchors):
            ...
            num_topk = min(topk_idxs.size(0), self.test_topk_candidates)
