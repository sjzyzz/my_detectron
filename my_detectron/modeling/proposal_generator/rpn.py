...
from my_detectron.modeling.proposal_generator.build import PROPOSAL_GENERATOR_REGISTRY
...
from my_detectron.modeling.sampling import subsample_labels
...
class RPN(nn.Module):
    ...
    def _subsample_labels(self, labels):
        """
        ...
        pos_idx, neg_idx = subsample_labels(
            labels, self.batch_size_per_image, self.positive_fraction, 0
        )
        # Fill with the ignore label (-1), then set positive and negative labels
        labels.fill(-1)
        labels.scatter(0, pos_idx, 1)
        labels.scatter(0, neg_idx, 0)
        return labels
    ...
    def losses(
        ...
        gt_labels = torch.stack(gt_labels)  # (N, sum(Ni*Wi*Ai))
